...
from desktop.watched.model import load as watched_load
...
def test_load_watched_entries_reloads_poster_cache(monkeypatch) -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_root:
        ...
        monkeypatch.setattr(watched_load.storage_data, "load_dataset", lambda: {"Fresh Show": movie})
        ...
        first_entries = watched_load.load_watched_entries()
        card_before = first_entries[0][2]
        assert card_before.get("poster_src") in (None, "")

        second_entries = watched_load.load_watched_entries()
